...
public class Main {
    ...
    public static void main(String[] args) throws AWTException, InterruptedException {
        String[] phrases = new String[]{"a","b","c","d"}; // add all items

        long sleeptimeOut = 10000; // komutlar arasi bekleme suresi
        long startTimeout = 10000; // 10 saniye baslamadan once

        for (int index = 0; index < phrases.length; index++) {
            if (!phrases[index].endsWith("\n"))
        ...
        while (true) {
            ...
            nextPhrase = phrases[timesRun % (phrases.length)];

            keyboard.type(nextPhrase);

            timesRun++;

            if (timesRun == phrases.length)
                timesRun = 0;
